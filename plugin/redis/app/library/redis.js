...
function install(config, app) {
    ...
    redis.lock = function(resource = 'redlock', ttl = 10000) {
        if (!Pool[REDLOCK]) {
            ...
            Pool[REDLOCK].on('clientError', err => {
                app.logger.error('A redis error has occurred:', err) // 链接终端
            })
        }
        return Pool[REDLOCK].lock(resource, ttl).then(lock => lock).catch(err => {
            // app.logger.error('[redis/redlock.js] err：超出队列最大长度', err)
    ...
    redis.engine = function(opt = {db: 0}) {
        ...
        const _config = Object.assign({}, config, opt)
        const tunnel = md5(JSON.stringify(_config))
        if (!Pool[tunnel] || !Pool[tunnel].connector.connecting) {
            Pool[tunnel] = new Redis(_config)
        }
        return Pool[tunnel]
    }
