...
module.exports = options => {
    ...
    return async function(ctx, next) {
        ...
        const {camelCase, primaryKeyAdaptor} = ctx.app.config.router
        ...
        /**
         * 获取主键（自动判断主键是否合规）
         * 默认使用 int 进行判断，也可以传入指定 primaryKeyAdaptor 进行判断
         * @param {string} sample 需要判断的样本字符
         * @return {number|boolean} 若判定成功返回正确的主键值
         */
        const getPrimary = sample => {
            if (sample) {
                if (typeof primaryKeyAdaptor === 'function') {
                    const primary = primaryKeyAdaptor(sample)
                    if (primary) {
                        return primary === true ? sample : primary
                    }
                } else {
        ...
        // 例: /user/5/address/6 解析为 /user/address?user=5&address=6
